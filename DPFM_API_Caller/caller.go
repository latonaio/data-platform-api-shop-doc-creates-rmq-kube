...
import (
	"context"
	"crypto/sha256"
	dpfm_api_input_reader "data-platform-api-shop-doc-creates-rmq-kube/DPFM_API_Input_Formatter"
	dpfm_api_output_formatter "data-platform-api-shop-doc-creates-rmq-kube/DPFM_API_Output_Formatter"
	"data-platform-api-shop-doc-creates-rmq-kube/config"
	"encoding/base64"
	"encoding/hex"
	"fmt"
...
func (c *DPFMAPICaller) AsyncDocCreates(
	...
	var fileExtension = input.Shop.HeaderDoc.FileExtension
	var docData = input.DocData

	var combinedString = fmt.Sprintf(
		"%v%v",
		time.Now().Unix(),
		input.Shop.HeaderDoc.DocVersionID,
	)
	...
	response := c.createSqlProcess(input, output, &dpfm_api_output_formatter.HeaderDoc{
		...
		DocVersionID:             input.Shop.HeaderDoc.DocVersionID,
		DocID:                    &dockId,
		FileExtension:            input.Shop.HeaderDoc.FileExtension,
		FileName:                 input.Shop.HeaderDoc.FileName,
		FilePath:                 directoryPath,
		DocIssuerBusinessPartner: input.Shop.HeaderDoc.DocIssuerBusinessPartner,
	}, errs, log)
